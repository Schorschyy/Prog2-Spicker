...
public class Lager extends Standort {
	...
	public void lagerSpeichern(ApplicationContext context) {
		Util u = new Util();
		File lagerDaten = new File(context.getDateipfad()+"Lager.txt");
		FileWriter fw = null;
		try {
			fw = new FileWriter(lagerDaten);
			BufferedWriter bw = new BufferedWriter(fw);
			bw.write(getOrt());
			for (Warenart w : lagerbestand.keySet()) {
				if (lagerbestand.get(w)>0) {
					bw.newLine();
					bw.write(Util.convertWarenartToString(w) + ";" + lagerbestand.get(w));
				}
			}	
			bw.close();
		} catch (IOException e) {
