...
public class CommandLoehneFestlegen implements Command {
	...
	public void execute(ApplicationContext context) {
		...
		System.out.println("========== In welcher Niederlassung sollen löhne festgelegt werden?");
		...
		if(auswahlNiederlassungIndex == indexNdls) {
		 ...
		 while(!scannerErfolg) {
				try {
					System.out.print("Wie Hoch soll der neue Lohn sein?");
					neuerLohn = myScanner.nextInt();
					if(neuerLohn >= 0) {
					scannerErfolg = true;
					}
					myScanner.nextLine();
		 ...
		 context.getNiederlassungen().get(auswahlNiederlassungIndex).setLohn(neuerLohn);
