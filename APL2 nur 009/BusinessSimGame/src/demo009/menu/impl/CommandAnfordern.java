...
public class CommandAnfordern implements Command {
	...
	public void execute(ApplicationContext context) {
		...
		if(context.getVerfuegbareFuhrwerke()==0) {
			System.out.println("Es sind keine Fuhrwerke mehr f�r das Senden zu Niederlassungen verf�gbar.");
			// Verlassen des Menus
			return;
		}
		...
		while(!scannerErfolg) {
			try {
				System.out.println("Es k�nnen nur maximal "+context.getVerfuegbareFuhrwerke()+" Fuhrwerk gesendet werden.");
		...
		while(!scannerErfolg) {
			try {
				...
				if(context.getVerfuegbareFuhrwerke()>=anzahlZuSendenderFuhrwerke) {
					...
					System.out.println("Es k�nnen nur maximal "+context.getVerfuegbareFuhrwerke()+" gesendet werden.");
					scannerErfolg=false;
				}
				myScanner.nextLine();
			} catch (Exception e) {
		...
		listeNiederlassung.get(auswahlNiederlassungIndex).setgesendetefuhrwerke(anzahlZuSendenderFuhrwerke);
		// Anforderung setzen
